...
**Addendum (a guest whose provider login is already an account).** The in-place upgrade above assumes the Google or Apple login is new to Waves. When it is not, `linkIdentity` is refused (`identity_already_exists`) and there is nothing to upgrade into: the two accounts cannot be fused without moving ledger rows from one user to another, which ADR-004 does not allow. So the person moves instead. They are offered a switch to the account they already have; the groups they joined as a guest by link are rejoined from that account through the ordinary invite path, and the guest leaves the ones it joined by link and never wrote to, so a group does not count the same person twice. Anything the guest did write stays with the guest, and the person is shown how much before they choose. See TDR A83.
...
**Addendum (the preferences are now actually read, and a settlement now tells the other party).** This decision is unchanged; what changed is that two halves of it had never been implemented, and the spec described the design rather than the code. Three of the four push switches on `notification_prefs` — `involvesMe`, `settlementRequests` and `groupActivityDigest` — were rendered on the notifications screen, saved to the profile, and read by nothing: turning one off had never stopped a single push, and `nudges` worked only by accident, because the trip-nudge job happened to check it. `waves_claim_push_notifications` now maps every kind to its switch (`waves_pref_key_for_kind`) and suppresses a refused row terminally, exactly as the email claim already did for its own. Separately, no function in the database had ever produced a `settlement_initiated` or `settlement_confirm_request` row, so the "payee push with Confirm action" in this ADR's own settlement flow had no producer, and the cancel and dispute of ADR-007 — written as mirror images so that "neither party can silently erase the other's record" — told nobody. Both directions now notify. A switch that renders and changes nothing is worse than a switch not offered, because it fails silently in the one direction nobody reports; recorded here because the ADR's promise, "only what involves me", is only as true as the code that reads the preference. See TDR A68.
...
**Addendum (an assistant is a client, and the answer to a client is this ADR).** An MCP server (`apps/agent-mcp`, A72) lets somebody's own AI assistant read and correct their ledger. It introduces no authorization model at all, which is the point: it signs in **as the person** — the same six-digit email code over stdio, an OAuth 2.1 + PKCE flow against Supabase Auth over HTTP — holds no service-role key, offers no raw-SQL or query tool, and reaches Postgres through PostgREST under RLS, six definer RPCs and `expense-write`. So an assistant can reach exactly what the person can reach and nothing else, and every policy written for the phone applies to it unread.

What is new is a **ceiling**, and it is a different kind of control from everything else here. Authorization answers "may you"; this answers "how much, before somebody looks". A token minted through Supabase's OAuth server carries a `client_id` claim the app's own tokens do not; `waves_agent_client_id()` reads it from the verified claims — never from an argument, which would be a claim by the caller about the caller — and returns NULL for the app, which is what keeps the app uncapped. `waves_assert_agent_cap` then refuses a single write over `app_config.agent_expense_cap_minor` or a rolling day over `agent_daily_cap_minor`, **per person rather than per token**, so minting a second token is not a way round the first. `agent_writes` records what was done and which client did it, with an owner-read policy and no INSERT policy at all — the rows come only from a definer function, so an assistant cannot file a trail naming somebody else or quietly file none. The numbers are starting values in `app_config` rather than a considered policy: the point is that the answer to "what stops a runaway loop" is a number somebody can raise deliberately, instead of "nothing". Two limits of it are worth stating rather than discovering: the ceiling is money-only, so making a group, adding people, minting a join link and deleting an expense sit outside it; and it does not cover the developer API above, whose short-lived session carries no `client_id` — whether it should is undecided.
