...
EXTENSIONS='ts|tsx|js|jsx|mjs|cjs|json|md|mdx|sql|prisma|ya?ml|xml|toml|css|html|svg|png|jpg|jpeg|webp|ico|ttf|otf|woff2?|sh|swift|kt|gradle|podspec|txt|example|gitignore|gitattributes|npmrc|nvmrc|prettierrc|prettierignore|vercelignore|py|editorconfig|lock'
