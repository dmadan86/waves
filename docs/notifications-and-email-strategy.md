...
| **P6c** | **Transports named per row (R3), section-level off (R4).** Screen only, no schema. R2 is already shipped — the permission card renders from `pushPermission()` on every load.                                                                                     | **JS only**                                              |
